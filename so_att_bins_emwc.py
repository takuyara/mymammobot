...
def get_parser():
	...
	parser.add_argument("--reg-dims", type = int, default = 3)
...
def main():
	...
	for epoch in range(args.epochs):
		...
		for phase, loader in [("train", train_loader), ("val", val_loader)]:
			...
			loss, acc, f1, l1 = sum_loss / num_loss, accuracy_score(y_true, y_pred), f1_score(y_true, y_pred, average = "macro"), np.mean(np.sum((coord_trues - coord_preds) ** 2, axis = -1))
			print("Epoch {} phase {}: loss = {:.4f}, accuracy = {:.4f}, f1 = {:.4f}, reg L2 = {:.4f}".format(epoch, phase, loss, acc, f1, l1), flush = True)
			if phase == "val":
				if l1 > max_acc:
					max_acc, max_f1, max_l1, best_weights = l1, f1, l1, deepcopy(model.state_dict())
