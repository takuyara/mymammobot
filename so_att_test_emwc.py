...
from utils.cl_utils import load_all_cls_npy, get_cl_dist_sum, axial_to_cl_point_ori, project_to_cl, in_mesh_bounds
...
def turn_emwc_to_coc(pos_list, print_oob = False):
	labels, depths = [], []
	num_oobs = 0
	for t_pos in pos_list:
		proj, (cl_idx, on_line_idx) = project_to_cl(t_pos, all_cls, return_cl_indices = True)
		depth = np.linalg.norm(all_cls[cl_idx][0][on_line_idx, ...] - proj)
		if on_line_idx > 0:
			depth += all_cl_sums[cl_idx][on_line_idx - 1]
		labels.append(cl_idx)
		depths.append(depth)
		if np.linalg.norm(t_pos - all_cls[cl_idx][0][on_line_idx, ...]) > all_cls[cl_idx][1][on_line_idx] + 0.5:
			num_oobs += 1
	print("OOB percentage {:.2f}".format(num_oobs / len(pos_list) * 100))
	return np.array(labels), np.array(depths)
...
def get_yt_yp_list(args, val_path, model):
	...
	print(np.mean(np.sum((coord_trues - coord_preds) ** 2, axis = -1) ** 0.5))
	print("Y true")
	y_true, coord_trues = turn_emwc_to_coc(coord_trues)
	print("Y pred")
	y_pred, coord_preds = turn_emwc_to_coc(coord_preds, print_oob = True)
