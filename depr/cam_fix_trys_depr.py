...
def check_rotation(frame_idx, em_path, em_depth_path, output_path, args):
	...
	def plot_it(rgb, virtual_depth_map, error_mode = "corr"):
		rd1, vd1 = real_depth_map.reshape(-1, 1), virtual_depth_map.reshape(-1, 1)
		w = np.ones(rd1.shape[0])
		w[np.logical_and(rd1.flatten() < 3, vd1.flatten() < 8)] = 0.4
		if error_mode == "linear":
			reg = LR().fit(rd1, vd1, w)
			score = reg.score(rd1, vd1, w)
			err = (vd1 - reg.predict(rd1)) ** 2
		elif error_mode == "corr":
			rd1, vd1 = rd1.flatten(), vd1.flatten()
			err = (w * (rd1 - weighted_mean(rd1, w)) * (vd1 - weighted_mean(vd1, w))) / (weighted_cov(rd1, rd1, w) * weighted_cov(vd1, vd1, w))
	...
	new_rgb, new_dep = get_depth_map(p, camera_params["focal_length"], new_position, new_orientation, new_up, get_outputs = True)
	err1 = plot_it(new_rgb, new_dep)
	old_rgb, old_dep = get_depth_map(p, camera_params["focal_length"], t_position, t_orientation, up_vector_base, get_outputs = True)
	err2 = plot_it(old_rgb, old_dep)
	plt.subplot(2, 2, 1)
	plt.imshow(err1, vmin = -0.5, vmax = 5)
	plt.colorbar()
	#plt.hist(err1.flatten())
	plt.title(f"New pose error: {comb_corr_sim(real_depth_map, new_dep):.4f}")
	plt.subplot(2, 2, 2)
	#plt.hist(err2.flatten())
	plt.imshow(err2, vmin = -0.5, vmax = 5)
	plt.colorbar()
	plt.title(f"Old pose error: {comb_corr_sim(real_depth_map, old_dep):.4f}")

	plt.subplot(2, 2, 3)
	plt.imshow(new_dep, vmin = 0, vmax = 40)
	plt.colorbar()
	plt.subplot(2, 2, 4)
	plt.imshow(old_dep, vmin = 0, vmax = 40)
	plt.colorbar()
	plt.show()
